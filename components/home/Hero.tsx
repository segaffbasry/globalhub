    ...
    const ctx = gsap.context(() => {
      const play = () => {
        ...
        gsap.timeline({ onComplete: () => words.revert() })
          .fromTo(".hero-window", { y: 90 }, { y: 0, duration: reveal.duration * 1.8, ease: reveal.ease, clearProps: "transform" }, 0)
  ...
  return <section className="hero" ref={stage} data-tone="light" aria-roledescription="carousel" aria-label="GlobalHUB at a glance">
    ...
    <div className="hero-window">
      <div className="hero-chrome hero-controls">
        ...
        <span className="hero-count" aria-hidden="true"><span>{pad(index + 1)}</span> / {pad(slides.length)}</span>
      ...
      <div className="hero-tabs hero-controls" role="group" aria-label="Choose a slide">
        {slides.map((slide, i) => <button key={slide.title} className={`hero-tab ${i === index ? "is-active" : ""} ${running ? "is-running" : ""}`} style={{ "--slide": `${SLIDE_MS}ms` } as React.CSSProperties} onClick={() => go(i)} aria-label={`Show slide ${i + 1}: ${slide.title}`} aria-current={i === index}>{slide.title}<span aria-hidden="true" /></button>)}
      </div>
